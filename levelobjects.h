...
{
  public:
    void resetDrawn() { drawn = false; }
    void trigInit(int tr, int tt) { trigger = tr; trigtype = tt; resetTrigger(); }
    void renewTrigger() { if (!(trigtype & TRIG_ONCE)) doTrigger = true; }
    void handleTrigger(bool bullet, bool player);
    void resetTrigger() { doTrigger = true; }
    bool drawn;
  protected:
    int trigger;
    int trigtype;
...
class GridCell
{
  public:
    ...
    void reset()
    {
      floor.clear(); wall.clear(); ceil.clear(); surf.clear();
      light.clear(); door.clear(); mask.clear();
    }
    ...
    void add(int objtype, int op)
    {
      switch(objtype)
      {
        case WALL:  wall.push_back(op);  break;
        case FLOOR: floor.push_back(op); break;
        case CEIL:  ceil.push_back(op);  break;
        case SURF:  surf.push_back(op);  break;
        case LIGHT: light.push_back(op); break;
        case DOOR:  door.push_back(op);  break;
        case MASK:  mask.push_back(op);  break;
      }
    }
...
class PathNode
{
  public:
    PathNode() { set(0, 0, 0); }
    PathNode(float x, float y) { set(0, x, y); }
    void set(int pr, float x, float y) { priority = pr; pos.set(x, y); }
    void addToVisibleSet(int num, float dst) { visible.push_back(num); distance.push_back(dst); }
    bool isVisible(int num) { for (int i = 0; i < visible.size(); i++) {if (i == num) return true;} return false; }
...
{
  public:
    Light() { drawn = false; color.zero(); pos.set(0,0); radius = 0; }
    Light(float r, float g, float b, float a, float x, float y, float rad, bool spot)
    { drawn = false; color.set(r,g,b,a); pos.set(x,y); radius = rad; lightSpot = spot; bb.set(Vector2D(x-radius,y-radius),Vector2D(x+radius,y+radius));}
    float Strength() { return color.a; }
    void draw();
...
class Wall : public Line, public LevelObject
{
  public:
    ...
    void set(float p1x, float p1y, float p2x, float p2y, float r, float g, float b, float a, float h, float frc, float hlth, int pt, float pq, bool lt, int cf, int tr, int tt)
      { drawn = false; p1.set(p1x, p1y); p2.set(p2x, p2y); setColor(r, g, b, a); force = frc; health = hlth; height = h; particleType = pt; particleQty = pq; lit = lt; collFlags = cf; trigInit(tr, tt); normal = findNormal(); bb.addPoint(p1); bb.addPoint(p2); }
    ...
    float getAlpha() { return baseColor.a; }
    ...
    float Force() { return force; }
    float Health() { return health; }
    float Height() { return height; }
    int CollFlags() { return collFlags; }
    bool intersectBB(BoundingBox b) { return bb.intersectBB(b); }
...
{
  public:
    Surface() { baseColor.zero(); litColor.zero(); vert.clear(); friction = 0; health = 0; trigInit(0, 0); }
    ...
    void setVert(int num, float x, float y) { if (num >= 0 && num < vert.size()) vert[num].set(x, y); }
    void setAlpha(float a) { litColor.a = a; }
    float Alpha() { return litColor.a; }
    void startFade(int dir) { fadeDirection = dir; }
    void fade();
    bool pointInBB(Vector2D point) { return bb.pointInBB(point); }
    bool intersectBB(BoundingBox b) { return bb.intersectBB(b); }
    float Friction() { return friction; }
    float Health() { return health; }
    int Trigger() { return trigger; }
    int CollFlags() { return collFlags; }
    bool Reobscure() { return reobscure; }

    void resetMask() { fadeDirection = 0; triggered = false; litColor.a = 1.0; }
