...
public:
    ...
    Animation( int nf, float dl, int lp )
    {
        set( nf, dl, lp );
    }
    void define( int nf, float dl, int lp )
    {
        numFrames = nf;
        delay = dl;
        loop = lp;
    }
    void set( int nf, float dl, int lp )
    {
        define( nf, dl, lp );
        reset();
    }
...
public:
    ...
    ModelAnimation( int mdl )
    {
        set( mdl );
    }
    void reset()
    {
        model = baseAnim = transientAnim = 0;
        currAnim = false;
        currFrame = 0;
        nextFrame = 1;
        animTime = 0.0;
...
public:
    ...
    Force( int obj, float dur, float a, float b, bool mode )
    {
        set( obj, dur, a, b, mode );
    }
...
public:
    ...
    {
        LR = FB = strafe = 0.0;
        jump = fire = grenade = reload = switchWpn = 0;
    }
    void setLR( float val )
    {
        LR = val;
    }
...
public:
    ...
    void clearSprites()
    {
        for( int i = 0; i < MAX_OBJ_SPRITES; i++ ) sprites[i] = NO_SPRITE;
    }
    void setSprites( int num, ... );
    void setModels( int mod1, int mod2 )
    {
        model1 = mod1;
        numModels = 1;
        if( mod2 != NO_MODEL )
...
public:
    BulletType() {}
    BulletType( float dmg, float f )
    {
        set( dmg, f );
    }
    void set( float dmg, float f )
    {
        damage = dmg;
        impactForce = f;
    }
...
public:
    LaserType() {}
    LaserType( int spr, float w, float mr, int mb, float dmg, float fs )
    {
        set( spr, w, mr, mb, dmg, fs );
    }
...
public:
    ...
    Object( int typ, float x, float y, float head, int rt1, int rt2, int tr, int tt )
    {
        setID();
        set( typ, x, y, head, rt1, rt2, tr, tt );
    }
    Object( int typ, float x, float y, float height, float head, int rt1, int rt2, int tr, int tt )
    {
        setID();
        set( typ, x, y, height, head, rt1, rt2, tr, tt );
    }
    ...
    void setHeading( float nhead )
    {
        heading = nhead;
    }
    void setLookAngle( float nangle )
    {
        lookAngle = nangle;
    }
    void turn( float amt );
    ...
    void setHVel( float nhvel )
    {
        hvel = nhvel;
    }
    void setTurnVel( float ntvel )
    {
        turnvel = ntvel;
    }
    void setActive( bool val )
    ...
    void setDirectionAlert( float ang, int typ )
    {
        dirAlert = ang;
        dirAlertType = typ;
    }
    ...
    float DirectionAlert()
    {
        return dirAlert;
    }
    float DirectionAlertType()
    {
        return dirAlertType;
    }
    int State()
    ...
    void resetShockCollisions()
    {
        shockCollisions = 0;
    }
    long long int ShockCollisions()
    {
        return shockCollisions;
    }
    ...
    void setObjectCollisionDetectOnOff( bool s )
    {
        collideWithObjects = s;
    }
protected:
    void drawStart();
    void drawEnd();
...
public:
    ...
    Entity( int typ, float x, float y, float head, int st, bool fix, int rt1, int rt2, int tr, int tt, bool aug ) : Object( typ, x, y, head, rt1, rt2, tr, tt )
    {
        fixed = fix;
        state = st;
        if( aug ) Augment();
    ...
    Weapon* Wpn()
    {
        return &wpn[currWeapon];
    }
    Vector2D* WeaponPoint()
    {
        return &wpnPos[currWeapon];
    }
...
public:
    ...
    Particle( int typ ) : Object( typ )
    {
        initType();
    }
    Particle( int typ, float x, float y, float head ) : Object( typ, x, y, head, 0, 0, 0, 0 )
    {
        initType();
    }
    Particle( int typ, float x, float y, float height, float head ) : Object( typ, x, y, height, head, 0, 0, 0, 0 )
    {
        initType();
    }
    void initType()
    {
        pType = type - PART_PARTICLE;
        color.white();
    }
...
public:
    Bullet() : Particle() {}
    Bullet( int own, int typ ) : Particle( typ )
    {
        owner = own;
        bType = type - BLT_RIFLE;
    }
    Bullet( int own, int typ, float x, float y, float head ) : Particle( typ, x, y, head )
    {
        owner = own;
        bType = type - BLT_RIFLE;
    }
    Bullet( int own, int typ, float x, float y, float height, float head ) : Particle( typ, x, y, height, head )
    {
        owner = own;
        bType = type - BLT_RIFLE;
    }
    void launch( int own, int typ, float x, float y, float height, float head )
    {
        ...
        owner = own;
        bType = typ - BLT_RIFLE;
        Particle::launch( typ, x, y, height, head, 1.0, 0 );
    }
