...
public:
    ...
    void reset()
    {
        ...
        styles.clear();
        rowStart = 0;
        line = ch = linesDisplayed = 0;
        drawTime = printTime = 0;
        upPr = downPr = enterPr = false;
    }
    void setCursor( int ct, int cb, int cc, int cp, string st, int ev );
    void calculateDimensions()
    {
        w = ( charWidth * cols ) + ( 2 * thick ) + charWidth;
        h = ( charHeight * rows * 1.5 ) + ( 2 * thick ) + charHeight / 1.5;
    }
    void draw();
    void deactivate()
    {
    ...
    void activate()
    {
        ...
        fullyDrawn = false;
        drawTime = 0.0;
        playSound( SND_TERM_SELECT, 3 );
    }
    void activate( Uint8* keystate );
    void flush()
    {
        fullyDrawn = true;
        line = linesDisplayed = lines.size();
    }
    ...
    void cursorUp()
    {
        curPos = ( curPos > curTop ) ? curPos - 1 : curBottom;
        playSound( SND_TERM_MOVE, 3 );
    }
    void cursorDown()
    {
        curPos = ( curPos < curBottom ) ? curPos + 1 : curTop;
        playSound( SND_TERM_MOVE, 3 );
    }
    int getCursorPos()
    {
    ...
    bool doneDrawing()
    {
        return fullyDrawn && linesDisplayed == lines.size();
    }
    bool isActive()
    {
        return active;
    }
    void resetKeys( Uint8* k )
    {
        upPr = k[SDLK_UP];
        downPr = k[SDLK_DOWN];
        enterPr = k[SDLK_RETURN];
        escPr = k[SDLK_ESCAPE];
    }
private:
    int rows, cols;
