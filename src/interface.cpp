...
void TerminalWindow::draw()
{
    if( active )
    {
        ...
        glTexCoord2f( 1.0, 0.0 );
        glVertex2f( x + w - thick, y - thick );
        glTexCoord2f( 1.0, tmpH );
        glVertex2f( x + w - thick, y - tmpH );
        glTexCoord2f( 0.0, tmpH );
        glVertex2f( x + thick, y - tmpH );
        glEnd();
...
void configScreen( bool persp, float& scroll1, float& scroll2 )
{
    ...
    while( !done )
    {
        ...
        if( currRes > -1 )
        {
            config.xres = abs( resolutions[2 * currRes] );
            config.yres = resolutions[2 * currRes + 1];
        }
    ...
    if( done == 1 ) // Cancel
    {
        config.xres = oldXres;
        config.yres = oldYres;
        config.depth = oldDepth;
        config.fullscreen = oldFS;
        config.blur = oldBlur;
        config.sfx = oldSFX;
        config.music = oldMusic;
    }
    else if( done == 2 ) // Save
    {
