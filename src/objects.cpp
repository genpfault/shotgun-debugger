                    ...
                    float imtv, float ia, float ita, float hlth, int dth, bool cd, bool grav, bool shad)
{
  ...
  boundRadius = ir;
  mass = im; maxVel = imv; jumpVel = ijv;
  maxTurnvel = imtv; accel = ia; turnAccel = ita;
  maxHealth = hlth;
  deathType = dth;
  collide = cd; gravAffect = grav; hasShadow = shad;
  model1 = model2 = 0;
  numModels = 0;
  clearSprites();
}
...
{    
  if (active)
  {
    ...
    if (!objType[type].numModels)
    {
      ...
      if (spr1 != NO_SPRITE) {texQuad(objType[type].sprites[spr1]);}
      glTranslatef(0.0, 0.0, 0.01);
      if (spr2 != NO_SPRITE) {texQuad(objType[type].sprites[spr2]);}
    }
    else
    {
      glEnable(GL_TEXTURE_2D);
...
void Object::levelCollisionDetection()
{
  if (currLevel.inGrid(pos))
  {
    ...
    { forceVel.normalize(); forceVel *= TERMINAL_VELOCITY; }
...
void Object::update()
{
  ...
  {    
    ...
    {  
      ...
      {
        if (surfaceHealthSound) { playSound(SND_HEAL, 2); surfaceHealthSound = false; }
      }
      else if (currSurfaceHealth < 0 && !config.cheat_invul)
      {
        if (surfaceHealthSound) { playSound(SND_HURTSURFACE, 2); surfaceHealthSound = false; }
      }
      else
...
void Object::dyingCheck()
{
  if (!dying && health <= 0 && health != INF)
  {
    switch(type)
    {
      ...
      case ENT_ARMED_DRONE: case ENT_HUNTER: case ENT_GLADIATOR:
...
void Object::doDeath()
{
  ...
  switch(objType[type].deathType)
  {
    ...
    case DEATH_GLADIATOR:
      ...
      break;
    case DEATH_ESG:
      px = PLAYER_OBJECT->PosX(); py = PLAYER_OBJECT->PosY();
...
void Entity::reset()
{
  ...
  switch(type)
  {
    case ENT_PLAYER:
      ...
      if (!augmented)
      {
        wpn[0].set(WPN_NONE, MAX_AMMO*(int)config.cheat_weapons, 0);    wpnPos[0].set(0,0);
        wpn[1].set(WPN_RIFLE, MAX_AMMO*(int)config.cheat_weapons, 0);   wpnPos[1].set(-0.25, 1.1);
        wpn[2].set(WPN_SHOTGUN, MAX_AMMO*(int)config.cheat_weapons, 0); wpnPos[2].set(-0.20, 1.1);
        wpn[3].set(WPN_HEPA, MAX_AMMO*(int)config.cheat_weapons, 0);    wpnPos[3].set(-0.35, 0.9);
        wpn[4].set(WPN_GRENADE, MAX_AMMO*(int)config.cheat_weapons, 0); wpnPos[4].set(-0.45, 0.9);
        wpn[5].set(WPN_LASER, MAX_AMMO*(int)config.cheat_weapons, 0);   wpnPos[5].set(-0.30, 0.9);
        wpn[6].set(WPN_ESG, MAX_AMMO*(int)config.cheat_weapons, 0);     wpnPos[6].set(-0.30, 0.9);
      }
      else
        wpn[0].set(WPN_BLASTER, MAX_AMMO, 0); wpnPos[0].set(-0.30, 0.9);
        
      break;
    case ENT_TURRET1: case ENT_SLAVE_TURRET:
      wpn[0].set(WPN_TURRET_GUN, MAX_AMMO, -1); wpnPos[0].set(0,2.2);
      break;
    case ENT_ARMED_DRONE:
      wpn[0].set(WPN_RIFLE, MAX_AMMO, -1); wpnPos[0].set(-1.6, 1.5);
      break;
    case ENT_HUNTER:
      wpn[0].set(WPN_RIFLE, MAX_AMMO, -1); wpnPos[0].set(-2.5, 0.4);
      wpn[1].set(WPN_RIFLE, MAX_AMMO, -1); wpnPos[1].set(2.5, 0.4);
      break;
    case ENT_GLADIATOR:
      wpn[0].set(WPN_SHOTGUN, MAX_AMMO, -1); wpnPos[0].set(1.6, 1.9);
      break;
...
void Entity::setPlayerAnimations()
{
  if (type == ENT_PLAYER && !dying)
  {
    ...
    if (ev.getFB() != 0 || ev.getStrafe() != 0)
    {
      switch(currWeapon)
      {
        case WPN_NONE: case WPN_GRENADE:
          model[1].setAnimation(MA_PT_RUN, ANIM_BASE);
          break;
        case WPN_RIFLE: case WPN_SHOTGUN: case WPN_LASER:
          model[1].setAnimation(MA_PT_RIFLERUN, ANIM_BASE);
          break;
        case WPN_HEPA: case WPN_ESG:
  ...
  if (active && !fall)
  {
    ...
    else
    {
      if (ev.getFB() > 0 || ev.getStrafe() != 0)
      {
        ...
        { spr1Anim.setDirection(ANIM_FWD); spr2Anim.setDirection(ANIM_FWD); }
      }
      else if (ev.getFB() < 0 || ev.getStrafe() != 0)
      { spr1Anim.setDirection(ANIM_REV); spr2Anim.setDirection(ANIM_REV); }
      else
      {
        if (objType[type].numModels)
        { if (!(Stunned() || Dying())) setAnimation(MA_STAND, ANIM_BASE); }
        else
        { spr1Anim.freezeAtFrame(0); spr2Anim.freezeAtFrame(0); }
      }
    }  
    ...
    {
      if (switchReady)
        { selectNextWeapon(); switchReady = false; }
    }
    else
      switchReady = true;
      ...
      if (ev.getFire() && !Dying())
      {
        ...
        else
        {
                ...
                (wpnPos[currWeapon].c[Y]*sin(lookAngle)), lookAngle, 4.5))
          {
            switch(currWeapon)
            {
              case WPN_RIFLE: case WPN_SHOTGUN:
...
void Particle::draw()
{
  if (active)
  {
    ...
    // screwed over by the depth test
    glTranslatef(0.0, 0.0, 0.01);
    if (objType[type].hasShadow) {texQuadSize(TEX_SHADOW, objType[type].spriteSize.c[X]*scale, objType[type].spriteSize.c[Y]*scale); }
    ...
    {
      if (spr1 != NO_SPRITE) {texQuadSize(objType[type].sprites[spr1], objType[type].spriteSize.c[X]*scale, objType[type].spriteSize.c[Y]*scale); }
      glTranslatef(0.0, 0.0, 0.01);
      if (spr2 != NO_SPRITE) {texQuadSize(objType[type].sprites[spr2], objType[type].spriteSize.c[X]*scale, objType[type].spriteSize.c[Y]*scale); }
    }
    else
    {
      if (spr1 != NO_SPRITE) {texQuadClip(objType[type].sprites[spr1], anim.Frame(), 1.0/anim.NumFrames());}
      glTranslatef(0.0, 0.0, 0.01);
      if (spr2 != NO_SPRITE) {texQuadClip(objType[type].sprites[spr2], anim.Frame(), 1.0/anim.NumFrames());}      
    }
...
void Laser::launch(int typ, float x, float y, float height, float head)
{
  ...
  while (distLeft > 0.001 && bounces < lasType[type].maxBounces)
  {
    ...
    {
      closestHitWall->handleTrigger(true, false);
      Vector2D dir = endPos-startPos; dir.normalize();
...
{	
  ...
  for (int i = 0; i < NUM_PART_TYPES; i++)
    switch(i)
    {
      ...
      case P_PART_PARTICLE: case P_PART_PARTICLE_SLIME: case P_PART_PARTICLE_SPARK:
        partType[i].set(0, 5, 25,   0, 0,  0.5, 0.1, 0, 0.0, 0.0, HIT_BOUNCE, true, 0,    1, 0, 0, 0);
        break;
      case P_PART_PARTICLE_WOOD: case P_PART_PARTICLE_METAL:
      ...
      case P_PART_PARTICLE_ENERGY:
        ...
        break;
        break;
      case P_PART_MF_RIFLE: case P_PART_MF_SHOTGUN:
        partType[i].set(0, 0, 0, 0, 0, 4.0, 0, 0, 0.0,  0.0, HIT_NOTHING, false, 0,    1, 0, 0, 0);
        break;
      case P_BLT_RIFLE: case P_BLT_SHOTGUN:
     ...
     case P_BLT_GRENADE:
        ...
        break;
     case P_PART_RIFLE_SHELL: case P_PART_SHOTGUN_SHELL:
