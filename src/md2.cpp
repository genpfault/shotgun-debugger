...
bool MD2Loader::ImportMD2( Model* pModel, const char* strFileName, int textureIndex )
{
    ...
    m_Header.magic            = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.version          = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.skinWidth        = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.skinHeight       = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.frameSize        = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.numSkins         = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.numVertices      = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.numTexCoords     = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.numTriangles     = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.numGlCommands    = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.numFrames        = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.offsetSkins      = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.offsetTexCoords  = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.offsetTriangles  = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.offsetFrames     = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.offsetGlCommands = ReadI32( &buf[ offset ] );
    offset += 4;
    m_Header.offsetEnd        = ReadI32( &buf[ offset ] );
    offset += 4;
...
void MD2Loader::ReadMD2Data()
{
    ...
    // Read in all the texture coordinates in one fell swoop
    {
        ...
        {
            m_pTexCoords[i].u = ReadI16( &buf[ offset ] );
            offset += 2;
            m_pTexCoords[i].v = ReadI16( &buf[ offset ] );
            offset += 2;
        }
    }
    ...
    // Read in the face data for each triangle (vertex and texCoord indices)
    {
        ...
        {
            m_pTriangles[i].vertexIndices[0]  = ReadI16( &buf[ offset ] );
            offset += 2;
            m_pTriangles[i].vertexIndices[1]  = ReadI16( &buf[ offset ] );
            offset += 2;
            m_pTriangles[i].vertexIndices[2]  = ReadI16( &buf[ offset ] );
            offset += 2;
            m_pTriangles[i].textureIndices[0] = ReadI16( &buf[ offset ] );
            offset += 2;
            m_pTriangles[i].textureIndices[1] = ReadI16( &buf[ offset ] );
            offset += 2;
            m_pTriangles[i].textureIndices[2] = ReadI16( &buf[ offset ] );
            offset += 2;
        }
    }
