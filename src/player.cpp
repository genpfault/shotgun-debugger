  ...
  while (getline(datafile, line, '\n'))
  {
    ...
    if (line != "")
    {
      ...
      else if (lineNum >= 1 && lineNum <= 1+NUM_LEVELS)
      {
        ...
        value = NEXT_TOK; levelRecords[lineNum-1].kills = S2I(value);
        value = NEXT_TOK; levelRecords[lineNum-1].shots = S2I(value);
        value = NEXT_TOK; levelRecords[lineNum-1].hits =  S2I(value);
      }
      ...
      else if (lineNum >= 4+NUM_LEVELS && lineNum <= 3+2*NUM_LEVELS)
      {
        ...
        value = NEXT_TOK; currStats[lineNum-(4+NUM_LEVELS)].kills = S2I(value);
        value = NEXT_TOK; currStats[lineNum-(4+NUM_LEVELS)].shots = S2I(value);
        value = NEXT_TOK; currStats[lineNum-(4+NUM_LEVELS)].hits =  S2I(value);      
      }
...
void Player::update()
{
  ...
  {
    if (hlReady)
    { headingLock = (headingLock) ? false : true; hlReady = false; }
  }
  else
    hlReady = true;
