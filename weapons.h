...
class WeaponType
{
  public:
    WeaponType() {}
    WeaponType(const char *nm, int btype, int spr, int stype, int snd, float sx, float sy, int cs, int ma, int bps, float rof, float rt, float j, float r) { set(nm,btype,spr,stype,snd,sx,sy,cs,ma,bps,rof,rt,j,r); }
    ...
    { strncpy(name, nm, 17); sound = snd; bulletType = btype; sprite = spr; shellType = stype; spriteSize.set(sx, sy); clipSize = cs; maxAmmo = ma; bulletsPerShot = bps; rateOfFire = rof; reloadTime = rt; jitter = j; recoil = r; }
...
class Weapon
{
  public:
    Weapon() {}
    Weapon(int typ, int own) { type = typ; owner = own; reset(); }
    Weapon(int typ, int ra, int own) { set(typ, ra, own); }
    void set(int typ, int ra, int own);
    void set(int ra);
    void reset() { rofTimer = reloadTimer = ammoInClip = reserveAmmo = 0; reloading = semiAutoLock = playChargeSound = playChargeLoop = false; }
    void draw();
    void update();
    ...
    int AmmoInClip() { return ammoInClip; }
    int ReserveAmmo() { return reserveAmmo; }
    bool Ready() { return readyToFire && !semiAutoLock; }
    float RofTimer() { return rofTimer; }
    float ReloadTimer() { return reloadTimer; }
