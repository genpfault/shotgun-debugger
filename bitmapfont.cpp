...
void BitmapFont::drawChar(char c)
{
  ...
  glBegin(GL_QUADS);
      glTexCoord2f( col*.0625,    row*.125);     glVertex2f(-0.5, 1.0);
      glTexCoord2f((col+1)*.0625, row*.125);     glVertex2f(0.5, 1.0);
      glTexCoord2f((col+1)*.0625, (row+1)*.125); glVertex2f(0.5, -1.0);
      glTexCoord2f( col*.0625,    (row+1)*.125); glVertex2f(-0.5, -1.0);
  glEnd();
}  
